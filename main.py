    ...
    try:
        while True:
            ...
            if user_input == "/dream":
                ...
                session.log_dream(success=len(memories) > before, result_len=len(memories) - before)
                continue
            ...
            stored = len(memories) > before
            session.log_ingest(stored=stored, text=user_input)
